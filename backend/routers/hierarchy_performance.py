...
from backend.schemas.monthly_performance import SuperiorAssignmentCreate, MonthlyEvaluationCreate
...
def my_team(current_user: User = Depends(verify_superior)):
    return _call(get_team, current_user)
...
def hierarchy_history(employee_id: int, admin: User = Depends(verify_admin)):
    db = SessionLocal()
    try:
        from backend.models import EmployeeSuperiorHistory
        rows = db.query(EmployeeSuperiorHistory).filter(EmployeeSuperiorHistory.employee_id == employee_id).order_by(EmployeeSuperiorHistory.effective_from.desc()).all()
        return [{
            "id": row.id,
...
def team_monthly_performance(current_user: User = Depends(verify_superior)):
    return _call(get_team_performance, current_user)
...
def performance_insights(
    ...
    return _call(get_insights, admin, performance_month, department_id, superior_id, employee_id, rank)
